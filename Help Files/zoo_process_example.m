...
%   conditons for: 
%   (1) maximum hip adduction in stance (Hip_ADD)
%   (2) knee flexion angle at foot-off (Knee_FLX)
%   (3) maximum medio-lateral ground reaction force (GRF_ML)
%
...
%   by thrid party statistical programs such as SPSS and R.
%
% Created by Philippe C. Dixon November 2013 
%
% Updated by Philippe C. Dixon January 11th 2016
...
%   ORIGINAL_START_FRAME and ORIGINAL_END_FRAME refer to the actual frames captured in Vicon.
%   The ORIGINAL_START_FRAME is considered the first frame in the Zoosystem. Thus, the 
%   CURRENT_START_FRAME indicates how many frames were cut from the start during partionning.
...
%   each dependent variable to be analysed (exploded into three mx1 subchannels). 
% - The 'zoosystem' metainformation channel is never removed by 'bmech_removechannel'.g
...
% User notes
% - Local events have been added to the event branch of the channels selected. Users can explore 
...
%        from the 'File' menu on the main figure window and edit the sizing options
...
%        data remain. 
%   (6)  One line for RGroundReactionForce_x of the Turn condition appears separate from the others. 
%        Left click on the trace to identify the trial (HC002D25.zoo).  We will see later why this
%        trace is different, but for now let's assume it is an outlier that should be removed. This 
%        could be done by deleting the file in a standard window explorer (or mac finder) window,
%        but the rest of the data (hip and knee angles) appear unaffected and should not be deleted. 
%        In ensembler, left click on the trial, press 'delete' on the  keyboard and select 'Delete \
%        Channel'. This will replace all line and event data in this channel with 999 values (check 
%        using grab).
%   (7)  Select 'Ensembler' --> 'Ensemble (SD)' and the 'Ensembler' --> 'combine data' to graph 
...
%   (9)  Finalize graphs by exploring the menu bar options or by selecting Edit, property editor on.
%   (10) Save the figure by selecting File, save fig or export to pdf format by selecting File, export.
...
%   visualization of 3D motion data. Out of the box, it can animate motion trials for plug-in gait 
%   data, but can be updated by advanced users for use with other datasets. 
...
%  (3) Choose 'lower-limbs' and then select a few markers to display from the list (e.g. 'RP1M',
%      'RP5M', and 'RTOE'). This will load a skeleton and markers associated with the trial. Director 
%      detects the position of force plates in the file and also displays them in the 3D environment. 
%      Select 'RHipAngles' from the top-left channel list and click 'Play' to start the animation. 
...
%  (2) Select Ensembler --> Ensemble (CI) then Ensembler, combine data to show a mean and confidence 
%      interval (CI) curve for each condition on a single axis. 
%  (3) Select 'Analysis' --> 'bar graph' to display these discrete data. 
...
eventval('fld',fld,'dim1',dim1,'dim2',dim2,'localevts',levts,...
         'globalevts',gevts,'ch',ch,'excelserver','on','ext','.xlsx')   
