...
% - further improvements to layout
...
switch action
    ...
    case 'start'
        ...
        midpt = ss(3:4)/2;
        fsize = [34 12];            % total figure
        spos = [.1 .1 6 9.8];
        lpos = [.1 .1 8 9.8];       % left panel containing '1,1','1,2',...
        rpos = [.1 .1 8 9.8];       % right panel needs to be wide

        asize = [14 10];             % middle
        okbutt = [2 .5 1.5 1.5];
        but = [1 1]; % arrow button size
        ...
        % Explantory text
        uicontrol('style','text','units','centimeter','position',[0 fsize(2)-3 8 2],'tag','source1',...
                  'string','Current axes','backgroundcolor',FigColor,'foregroundcolor','k','FontSize',...
                   Settings.FontSize+2);
         % Explantory text
        uicontrol('style','text','units','centimeter','position',[fsize(1)-8 fsize(2)-3 8 2],'tag','source1',...
                  'string','New axes','backgroundcolor',FigColor,'foregroundcolor','k','FontSize',...
                   Settings.FontSize+2);
                 
        % Left panel         
        uicontrol('style','listbox','units','centimeter','position',lpos,'tag','source1',...
                  'string',fld1,'backgroundcolor',BgColor,'foregroundcolor',FgColor,'FontSize',...
                   Settings.FontSize);
        % Right Panel
        uicontrol('style','listbox','units','centimeter','position',[fsize(1)-rpos(1)-rpos(3) rpos(2:4)],...
                  'tag','source2','string',fld2,'backgroundcolor',BgColor,'foregroundcolor',[0 1 0],...
                  'FontSize',Settings.FontSize);
        % Right arrow button
        uicontrol('style','pushbutton','units','centimeters','position',[spos(1)+spos(3)+2.5 fsize(2)/2-.25 but],...
                  'string','>>','callback','associatedlg(''transfer'')','tag','source1');
        % Left arrow button
        uicontrol('style','pushbutton','units','centimeters','position',[fsize(1)-spos(1)-spos(3)-2.5-0.7,fsize(2)/2-.25 but],...
        ...
        ok = uicontrol('style','pushbutton','units','centimeters','position',...
                       [fsize(1)-spos(1)-spos(3)-okbutt(1)-okbutt(3) okbutt(2:4)],'callback',...
                       'delete(gcbo)','tag','ok','string','OK');
