...
for i=1:length(cons)-1
    ...
    if isempty(u)
        ...
        mdiffdatastk = [mdiffdata; mean(diffdata)];
        
        if ~isempty(check)
            [calpha_b, CIlow_b, CIhigh_b] = bootstrap_t(diffdata,nboots,alpha,[ch,' ',compcons]);   % bootstrap algorithm
        else
            [calpha_b, CIlow_b, CIhigh_b] = bootstrap_t(diffdata,nboots,alpha);   % bootstrap algorithm
        end
