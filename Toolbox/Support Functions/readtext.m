...
%
% text:     the file to be read. May be a file path or just the file name.
...
for n=1:nEnd
    ...
    if ~istextual
        ...
        if(lenT > 0)
            ...
            [a, count, errmsg, nextindex]=	sscanf(temp, '%f %1[ij] %1[+-] %f', 4);  % THIS IS THE BOTTLENECK LINE
...
if ~strcmp(opts.format, 'textual')
    nums= nums(1:(r-1), 1:result.max);
end		% In case we started off to big.
