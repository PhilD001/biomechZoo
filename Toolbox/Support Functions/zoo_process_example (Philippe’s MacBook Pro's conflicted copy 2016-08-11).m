...
%
% - removes channels (not used in the current study)
% - removes 'vsk' folder (not used in the current study)
...
ch = {'LFHD','LBHD','RFHD','RBHD','C7','T10','T12','RBAK','CLAV','STRN',...  
      ...
      'SACR','RASI','LASI','LTHI','LTIB','LKNE','LANK','LHEE','LTOE','RTHI',...
      'RTIB','RKNE','RANK','RHEE','RTOE','RHJC','LHJC','RFEO','LFEO','RTIO',...
      'LTIO','LPelvisAngles','LHipAngles','LKneeAngles','LAnkleAngles',...
      ...
      'RKneeForce','RAnkleForce','RHipMoment','RKneeMoment','RAnkleMoment',...
      'RHipPower','RKneePower','RAnklePower','LGroundReactionForce',...
      'LGroundReactionMoment','RGroundReactionForce','RGroundReactionMoment',...
      'ForceFx1','ForceFy1','ForceFz1','MomentMx1','MomentMy1','MomentMz1',...
      'ForceFx2','ForceFy2','ForceFz2','MomentMx2','MomentMy2','MomentMz2'};
    
bmech_removechannel(fld,ch,'keep')                  

bmech_removefolder(fld,'vsk')                                                 % rm folder
...
evtt1 = 'FS_FP';                                                             % start evt type
evtt2 = 'FO_FP';                                                             % end evt type
...
% just for now, delete this step in final draft
% ch1 = {'RightFootAngVelMag','LeftFootAngVelMag','RightShankAngVelMag',...
%     'LeftShankAngVelMag','RightThighAngVelMag','LeftThighAngVelMag','RightPelvisAngVelMag',...
%     'LeftPelvisAngVelMag','RightTz','LeftTz'}';
% ch2 = explodelist({'C7','CLAV','COP1','COP2','LANK','LASI',...   
%     'LAnkleForce','LAnkleJC','LBHD','LELB','LFEA','LFEL','LFEO','LFEP','LFHD','LFIN',...
%     'LFOA','LFOL','LFOO','LFOP','LGroundReactionMoment','LHEE','LHJC','LHipForce',...  
%     'LHipJC','LKNE','LKneeForce','LKneeJC','LSHO','LTHI','LTIA','LTIB','LTIL','LTIO',...  
%     'LTIP','LTOE','LWRA','LWRB','LeftAnkleForce','LeftCentreOfPressure','LeftFemurcom',... 
%     'LeftFootcom','LeftGroundReactionMoment','LeftHipForce','LeftKneeForce','LeftTibiacom',...   
%     'PELA','PELL','PELO','PELP','Pelviscom','RANK','RASI','RAnkleForce','RAnkleJC','RBAK',...
%     'RBHD','RELB','RFEA','RFEL','RFEO','RFEP','RFHD','RFIN','RFOA','RFOL','RFOO','RFOP',...  
%     'RGroundReactionMoment','RHEE','RHJC','RHipForce','RHipJC','RKNE','RKneeForce',...
%     'RKneeJC','RSHO','RTHI','RTIA','RTIB','RTIL','RTIO','RTIP','RTOE','RWRA','RWRB',...  
%     'RightAnkleForce','RightCentreOfPressure','RightFemurcom','RightFootcom',...
%     'RightTibiacom','SACR','STRN','T10','T12'});
% ch = [ch1;ch2];
...
if strfind(mode,'auto')
    ...
    outlier(out_file_turn,ch)                % walking in global Y negative direction
    outlier(out_file_straight,ch)            % walking in global Y negative direction
end
...
ch    = {'RightGroundReactionForce_x','RightHipAngle_y',...
         'RightKneeMoment_x','RightAnklePower'}; % chns to search
...
[~,pval_GRF_ML] = ttest(r.Straight,r.Turn,0.05,'both');                   % p-val = 0.005*
disp(['p-value for GRF_ml = ',sprintf('%.4f',pval_GRF_ML)])
disp(['GRF_ML Straight = ',sprintf('%.1f',nanmean(r.Straight)),...
    ' +/- ',sprintf('%.1f',nanstd(r.Straight))])
disp(['GRF_ML Turn = ',sprintf('%.1f',nanmean(r.Turn)),...
    ' +/- ',sprintf('%.1f',nanstd(r.Turn))])
...
disp(['p-value for Hip_ADD = ',sprintf('%.4f',pval_Hip_ADD)])
...
evt = 'RFO';
r = extractevents(fld,dim1,dim2,ch,evt);
[~,pval_Knee_FLX] = ttest(r.Straight,r.Turn,0.05,'both');                  % p-val = 0.033*
disp(['p-value for Knee_FLX = ',sprintf('%.4f',pval_Knee_FLX)])
...
disp(['p-value for Ankle_PWR = ',num2str(pval_Hip_ADD)])
