...
disp(' (3) kinemat: Integration of the kinemat toolbox to compute joint angles');
disp(' (4) Mac: Functions to implement general fixes for MAC OS');
disp(' (5) PiG Ops: Functions to be used with data containing Plug-in gait markers');
disp(' (6) Statistics: Functions to run statistics within Matlab and export event data to statistical software');
disp(' (7) Support Functions: Function in support of main zoosystem suites');
disp(' (8) Visualization: Functions mainly controlling GUIs (Director and Ensembler)');
disp(' (9) Zoo Conversions: Functions to convert data to zoosystem format (.zoo)');
disp(' (10) Zoo Processing: Functions to perform processing of zoo files');
disp(' (i) to output information about the the Zoosystem Toolbox');
disp(' (q) to quit Zoosystem Toolbox summary');

disp(' ')

selection = input('please make a selection for further instructions: ','s');
...
while selection ~= 0
    ...
    switch selection
        ...
        case '1'
            ...
            disp(' ')
            selection  = num2str(input('to make another selection type appropriate number: ','s'));
        ...
        case '3'
            ...
            disp(' (3) kinemat: Integration of the kinemat toolbox to compute joint angles');
            disp(' ')
            disp('bmech_kinemat: Computes lower-limb joint angles ')
            disp(' ')
            disp('For additional information regarding a specific function, type "help functionname" in the command prompt')
            disp(' ')
            selection  = num2str(input('to make another selection type appropriate number: ','s'));
        ...
        case '4'
          ...
          case '5'
            ...
            disp(' (5) PiG Ops: Functions to be used with data containing Plug-in gait markers');
            disp(' ')
            disp('bmech_jointcentrePiG: Computes joint centres for data containing PiG markers ')
            disp('vicon2groodsuntay: Computes joint kinematics via the Grood and Sunday method for PiG data')
            disp(' ')
            disp('For additional information regarding a specific function, type "help functionname" in the command prompt')
            disp(' ')
            selection  = num2str(input('to make another selection type appropriate number: ','s'));
